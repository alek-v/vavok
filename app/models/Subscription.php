...
class Subscription extends BaseModel {
    ...
    {
        $this->page_data['page_title'] = '{@localization[subscriptions]}}';
        ...
        if ($this->postAndGet('action') == "poddel") {
            $this->page_data['content'] .= '<p>{@localization[delallsub]}?</p>';
            $this->page_data['content'] .= $this->sitelink(HOMEDIR . 'adminpanel/subscriptions/?action=delallsub', $this->localization->string('yessure'), '<p><b>', '</b></p>');
            $this->page_data['content'] .= $this->sitelink(HOMEDIR . 'adminpanel/subscriptions', $this->localization->string('back'), '<p>', '</p>');
        } 
        ...
        if (empty($this->postAndGet('action'))) {
            ...
            if ($num_items > 0) {
                ...
                foreach ($this->db->query($sql) as $item) {
                    $this->page_data['content'] .= $this->sitelink(HOMEDIR . 'users/u/' . $item['user_id'], $this->user->getNickFromId($item['user_id']), '<b>', '</b>');
                    $this->page_data['content'] .= '<b><font color="#FF0000"> (' . $item['user_mail'] . ')</font></b>';
    
                    if (!empty($item['subscription_name'])) {
                        $this->page_data['content'] .= ' <strong>' . $item['subscription_name'] . '</strong>';
                    }
    
                    $this->page_data['content'] .= ' ' . $this->sitelink(HOMEDIR . 'adminpanel/subscriptions/?action=delmail&users=' . $item['user_id'], '[{@localization[delete]}}]') . '<hr>';
                }

                $this->page_data['content'] .= $navigation->getNavigation();
            } else {
                $this->page_data['content'] .= '<p><img src="/themes/images/img/reload.gif" alt="" /> No subscriptions</p>'; // update lang
            }
        }
    ...
    {
        $this->page_data['page_title'] = '{@localization[subscription_options]}}';
        ...
        if (empty($all_options)) {
            $this->page_data['content'] .= '<img src="' . HOMEDIR . 'themes/images/img/reload.gif" /> There is no subscription option defined';
        }

        // Show options
        $this->page_data['all_options'] = '';
        foreach($all_options as $key => $val) {
            $this->page_data['all_options'] .= '<div class="row mt-3 bg-light rounded d-flex align-items-center">';
            $this->page_data['all_options'] .= '<div class="col-2 m-2">{@localization[option]}}: ' . $key . '</div>';
            $this->page_data['all_options'] .= '<div class="col-3 m-2">{@localization[description]}}: ' . $val . '</div>';
            $this->page_data['all_options'] .= '<div class="col-2 m-2"><a href="./subscription_options?delete=' . $key . '" class="btn btn-danger">{@localization[delete]}}</a></div>';
            $this->page_data['all_options'] .= '</div>';
        }
