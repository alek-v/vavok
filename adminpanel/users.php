...
if ($vavok->post_and_get('action') == 'edit') {
    if (!empty($user) && $vavok->go('users')->username_exists($user) && $vavok->go('users')->id_exists($users_id)) {
        ...
        if (($vavok->go('users')->show_username() != $vavok->get_configuration('adminNick')) && ($vavok->go('users')->user_info('perm', $users_id) == 101 || $vavok->go('users')->user_info('perm', $users_id) == 102 || $vavok->go('users')->user_info('perm', $users_id) == 103 || $vavok->go('users')->user_info('perm', $users_id) == 105) && $vavok->go('users')->show_username() != $user) {
        ...
        if ($vavok->go('users')->user_info('banned', $users_id) == 1) echo '<p><font color="#FF0000"><b>' . $vavok->go('localization')->string('confban') . '</b></font></p>';

        if ($vavok->go('users')->user_info('regche', $users_id) == 1) echo '<p><font color="#FF0000"><b>' . $vavok->go('localization')->string('notactivated') . '</b></font></p>';

        $form = new PageGen('forms/form.tpl');
        $form->set('form_method', 'post');
        $form->set('form_action', 'users.php?action=upgrade&amp;users=' . $user);

        $userx_access = (int)$vavok->go('users')->user_info('perm', $users_id);
        ...
        $udd2->set('input_value', $vavok->go('users')->user_info('city', $users_id));
        ...
        $udd3->set('input_value', $vavok->go('users')->user_info('about', $users_id));
        ...
        $udd4->set('input_value', $vavok->go('users')->user_info('email', $users_id));
        ...
        $udd5->set('input_value', $vavok->go('users')->user_info('site', $users_id));
        ...
        $udd13->set('input_value', $vavok->go('users')->user_info('browser', $users_id));
        ...
        $udd29->set('input_value', $vavok->go('users')->user_info('firstname', $users_id));
        ...
        $udd40->set('input_value', $vavok->go('users')->user_info('status', $users_id));

        if ($vavok->go('users')->user_info('subscribed', $users_id) == 1) {
        ...
        $allban->set('input_placeholder', (int)$vavok->go('users')->user_info('allban', $users_id));
        ...
        $lastvst->set('input_placeholder', $vavok->date_fixed($vavok->go('users')->user_info('lastvisit', $users_id), 'j.m.Y. / H:i'));

        $ip = new PageGen('forms/input_readonly.tpl');
        $ip->set('label_for', 'ip');
        $ip->set('label_value', 'IP');
        $ip->set('input_id', 'ip');
        $ip->set('input_placeholder', $vavok->go('users')->user_info('ipaddress', $users_id));
...
if ($vavok->post_and_get('action') == 'deluser') {
    if ($user != $vavok->get_configuration('adminNick')) {
        if ($vavok->go('users')->user_info('perm', $users_id) < 101 || $vavok->go('users')->user_info('perm', $users_id) > 105) {
