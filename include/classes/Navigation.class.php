...
class Navigation {
	...
	public function __construct($items_per_page, $total_items, $link = '', $link2 = '')
	{
		...
		// Get page number
		$page = $this->vavok->post_and_get('page');
	...
	public function get_navigation($link = '', $lnks = 3)
	{
	    ...
	    if ($page > 1 && $this->total_pages > 1) {
	    	...
	    	$navigation .= $page == 2 ? $this->show_link($link, $this->vavok->go('localization')->string('prev')) : $this->show_link($link . 'page=' . ($page - 1), $this->vavok->go('localization')->string('prev'));
	    } else {
		    $navigation .= $this->disabled_link($this->vavok->go('localization')->string('prev'));
	    }
	    ...
	    if ($total > ($this->items_per_page * $page)) {
	        $navigation .= $this->show_link($link . 'page=' . ($page + 1), $this->vavok->go('localization')->string('next'));
	    }
	    // Disabled link when current page is last page
	    else {
		    $navigation .= $this->disabled_link($this->vavok->go('localization')->string('next'));
	    }
