...
if (!empty($vavok->post_and_get('subdel'))) {
    ...
    if ($vavok->post_and_get('subdel') == $email_check['user_pass'] && (empty($email_check['subscripton_name']) || $email_check['subscripton_name'] == $vavok->post_and_get('sn'))) {
        ...
        echo $vavok->show_notification('<p>' . $vavok->go('localization')->string('hello') . ' ' . $uz_log . '!</p>
                                        <p>' . $vavok->go('localization')->string('delsubok') . '!</p>');
    } else {
        echo $vavok->show_danger($vavok->go('localization')->string('unsubcodefail')); // code does not match
    }
} else {
    echo $vavok->show_danger($vavok->go('localization')->string('unsubfail')); // code 
}
